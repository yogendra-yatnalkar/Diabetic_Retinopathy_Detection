...
def crop_black(image):
    ...
    print(x1,y1,x2,y2)
    # crop = org[y1+2*y1:y2-2*y1, x1+2*x1:x2-2*x1]
    ext_x = (x2 - x1)*4//100
    ext_y = (y2 - y1)*4//100
    print(ext_x,ext_y)
...
def subtract_median_bg_image(im):
    ...
    displayImage("Median of clahe",bg)
    sub_med = cv2.addWeighted (im, 1, bg, -1, 255)
    return sub_med

def colorEnhancement(image1,image2):
    # thresh = cv2.threshold(image2, 100, 255, cv2.THRESH_TOZERO)[1]
    # displayImage('thresh',thresh)
...
def imageAugSave(path,img1,img2,img3,img4,img_ht,img_wd):
    ...
    # cv2.imwrite('new_train/25.png',img1)
    # cv2.imwrite('new_train/26.png',img2)
    # cv2.imwrite('new_train/27.png',img3)
    # cv2.imwrite('new_train/28.png',img4)
...
def main():
    ...
    # imageAugSave('new_train_2',image_final, aug1, aug2, aug3, img_ht, img_wd)
    # final_grey = cv2.cvtColor(image_final,cv2.COLOR_BGR2GRAY)
    # displayImage('Final grey',final_grey)
    # final_scaled = image_final/255
    # displayImage("Sacled final image",final_scaled)
    # median_final = cv2.medianBlur(image_final, 5)
    # displayImage('median',median_final)
    # gaussian_final = cv2.GaussianBlur(image_final, (5, 5), 0)
    # displayImage('gaussian',gaussian_final)
    end = time.time()
